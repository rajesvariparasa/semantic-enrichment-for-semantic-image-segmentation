...
def train_epoch_ssl(model, data, optimizer, ssl_type, criterion_t1,criterion_t2, metric_t1, metric_t2, siam_segment_bandnum,siam_segment_classes, log_var_seg , log_var_rec, omega, device):
    ...
    return avg_loss, avg_loss_t1, avg_loss_t2, avg_metric, avg_metric_t1, avg_metric_t2, log_var_seg.item(), log_var_rec.item()
...
def train_model_ssl(model, fold, train_data, val_data, ssl_type, batch_size, optimizer, scheduler, criterion_1,criterion_2, metric_t1, metric_t2, siam_segment_bandnum, siam_segment_classes, omega, device, patience,out_paths, epochs,log_var_seg, log_var_rec , ssl_init_args):
    ...
    for epoch in range(epochs):
        ...
        log_var_rec_history.append(log_var_rec_updated)
        log_var_seg_history.append(log_var_seg_updated)
        ...
        if val_epoch_loss < min_val_loss:
            ...
            best_epoch_metrics= {'train_loss':train_epoch_loss,
                                ...
                                'log_var_seg':log_var_seg_updated,
                                'log_var_rec':log_var_rec_updated
                                 }
...
def save_training_curves_ssl(best_epoch_nums, fold_histories, fold_metrics, cross_val_metrics, out_path):   
    ...
    if np.sum(fold_histories['fold_0']['train_acc_history_t1']) == 0 or np.sum(fold_histories['fold_0']['train_acc_history_t2']) == 0:
        ...
        for ax in axs:
            ax.tick_params(axis='both', which='major', labelsize=12)
        max_x_ticks = max([len(history['log_var_seg_history']) for history in fold_histories.values()])
        axs[0].set_xticks(np.arange(0, max_x_ticks, step = 5))
        plt.tight_layout()
        plt.savefig(os.path.join(out_path, 'log_variance_ssl.png'), dpi=800, pad_inches=0.2, bbox_inches='tight')
        plt.close()
